...
export default {
    ...
    methods: {
        ...
        //METODO PARA CREAR RUTINA EN LA BD (METHODS submit)
        async crearRutina() {
            const res = await db.collection('rutinasPersonalizadas').add({
                cliente: this.id,
                nombre: this.nombre,
                desc: this.desc,
                listaEjercicios: this.listaEjercicios
            })
            router.push({name: 'ClienteVer', params: {id: this.id}})
            console.log(res.id);
        },
        //METODO PARA AÑADIR UN EJERCICIO A LA LISTA (METHODS click)
        aniadirEjercicio() {
